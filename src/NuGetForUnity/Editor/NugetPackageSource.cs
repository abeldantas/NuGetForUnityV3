...
namespace NugetForUnity
{
    ...
    public class NugetPackageSource
    {
        ...
        /// <summary>
        /// Gets a value indicating whether the feed is a v3 feed.
        /// </summary>
        public bool Isv3
        {
            get
            {
                // Perform a HEAD request to the URL and check the response headers for "X-NuGet-Feed-Version: 3.0.0"
                try
                {
                    HttpWebRequest request = (HttpWebRequest)WebRequest.Create(ExpandedPath);
                    request.Method = "HEAD";
                    using (HttpWebResponse response = (HttpWebResponse)request.GetResponse())
                    {
                        string nuGetFeedVersion = response.Headers["X-NuGet-Feed-Version"];
                        return nuGetFeedVersion != null && nuGetFeedVersion.StartsWith("3.0.0");
                    }
                }
                catch (Exception)
                {
                    return ExpandedPath.Contains("v3");
                }
            }
        }
        ...
        public List<NugetPackage> FindPackagesById(NugetPackageIdentifier package)
        {
            ...
            {
                if (Isv3)  // v3 feed handling
                {
                    string url = string.Format("{0}/{1}/index.json", ExpandedPath, package.Id);
            ...
            using (var responseStream = NugetHelper.RequestUrl(url, username, password, timeOut: 5000))
            {
                using (var reader = new StreamReader(responseStream))
                {
                    if (Isv3)
                    {
                        ...
                        {
                            var package = NugetPackage.FromJson(item);
                            package.PackageSource = this;
                    ...
                    {
                        // v2 feed handling
                        packages = NugetODataResponse.Parse(XDocument.Load(reader));
